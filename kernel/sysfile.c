...
uint64 sys_mmap(void) {
    ...
    if (argaddr(0, &addr) < 0 || argaddr(1, &length) < 0 || argint(2, &prot) < 0 || argint(3, &flags) < 0 || argfd(4, &fd, &f) < 0 || argaddr(5, &offset) < 0 || length == 0)
        return -1;
    ...
    for (int i = 0; i < NVMA; i++) {
        ...
        } else if (pv->vastart < vaend) {
            vaend = PGROUNDDOWN(pv->vastart);
        }
    }
...
struct vma *findvma(struct proc *p, uint64 va) {
    for (int i = 0; i < NVMA; i++) {
        struct vma *pv = &p->vmas[i];
        if (pv->valid == 1 && va >= pv->vastart && va < pv->vastart + pv->length) {
            return pv;
        }
    }
    return 0;
...
uint64 sys_munmap(void) {
    ...
    if (addr <= v->vastart && addr + length > v->vastart) { // unmap at the beginning
        v->offset += addr + length - v->vastart;
        v->vastart = addr + length;
    }
    v->length -= length;
